					...
					{if $config['enable_auto_detect_ban']===true}
                        ...
                        {else}
                            ...
                            {foreach $config['auto_detect_ban'] as $key => $value}
                                {if $value['type']=='kill'}
                                <dt><i class="material-icons icon-sm">info_outline</i> 每当您触碰审计规则达 <strong>{$key}</strong> 次，将会被直接<strong>删除账号</strong></dt>
                                {else}
                                <dt><i class="material-icons icon-sm">info_outline</i> 每当您触碰审计规则达 <strong>{$key}</strong> 次，将会被封禁 <strong>{$value['time']}</strong> 分钟</dt>
                                {/if}
                            {/foreach}
                            </dl>
                        {/if}
