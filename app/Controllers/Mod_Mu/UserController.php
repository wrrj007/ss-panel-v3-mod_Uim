...
class UserController extends BaseController
{
    ...
    public function index($request, $response, $args)
    {
        ...
        if (in_array($node->sort, [0, 10]) && ($node->mu_only != -1 || in_array($node->id, $_ENV['commonPortsOffset']))) {
        ...
        foreach ($users_raw as $user_raw) {
            ...
            if ($mu_port_migration === true) {
                // 下发偏移后端口
                if ($muPort['type'] != 0 && (in_array($node->id, $_ENV['commonPortsOffset']) || $user_raw->is_multi_user != 0)) {
                    $user_raw->port = ($user_raw->port + $muPort['type']);
                }
                if ($muPort['type'] == 0 && $user_raw->is_multi_user != 0 && in_array($user_raw->port, array_keys($muPort['port']))) {
